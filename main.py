...
def create_conversational_chain(query: str, session_id):
    ...
    if 'order' in category[0]:
        relevant_data = order_data_retrieve(user_query)
        print("Relevant data:", relevant_data)
    elif 'inventory' in category[0]:
        if any(keyword in user_query.lower() for keyword in ["full","all","complete", "wood", "metal"]):
            if any(keyword in user_query.lower() for keyword in ["complete", "all", "full","los angeles","indiana","pennsylvania", "wood", "metal"]):
                compl_inv = fetch_inventory(user_query)
            else:
...
def handle_mention(event, say):
    ...
    conversational_rag_chain, inv = create_conversational_chain(text, session_id)
