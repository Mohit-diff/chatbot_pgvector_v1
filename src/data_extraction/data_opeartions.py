        ...
        try:
            ...
            with psycopg.connect(CONNECTION_STRING) as conn:
                ...
                with conn.cursor() as cursor:
                    ...
                    elif location is None and any(keyword in user_query.lower() for keyword in ["full","all","complete", "total", "entire"]):
                        results = []
                        for inv_json, sku in rows:
                            try:
                                data = json.loads(inv_json)  # Parse the JSON data
        ...
        try:
            ...
            with psycopg.connect(CONNECTION_STRING) as conn:
                ...
                with conn.cursor() as cursor:
                    ...
                    if location is None and any(keyword in user_query.lower() for keyword in ["full","all","complete", "total", "entire"]):
                        results = []
                        for inv_json, sku in rows:
                            try:
                                data = json.loads(inv_json)  # Parse the JSON data
            ...
            print(f"Database error: {db_error}")
            return []
    elif any(keyword in user_query.lower() for keyword in ["total", "entire","complete", "full", "all","los angeles","indiana","pennsylvania"]):
        location = next((loc for loc in VALID_LOCATIONS if loc.lower() in user_query), None)
        try:
            # Establish a connection using the connection string
            with psycopg.connect(CONNECTION_STRING) as conn:
